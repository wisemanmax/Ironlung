...
export const Cookie={
  ...
  set:(name,value,days=30)=>{const exp=new Date(Date.now()+days*864e5).toUTCString();const domainAttr=window.location.hostname.endsWith("ironlog.space")?";domain=.ironlog.space":"";document.cookie=`${name}=${encodeURIComponent(value)};expires=${exp}${domainAttr};path=/;secure;samesite=lax`;},
  clear:(name)=>{const domainAttr=window.location.hostname.endsWith("ironlog.space")?";domain=.ironlog.space":"";document.cookie=`${name}=;expires=Thu, 01 Jan 1970 00:00:00 GMT${domainAttr};path=/;secure`;},
};
